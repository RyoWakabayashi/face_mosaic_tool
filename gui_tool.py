...
class AdvancedFaceMosaicGUI:
    ...
    def setup_ui(self):
        ...
        # 入力ディレクトリ選択
        ttk.Label(main_frame, text="入力ディレクトリ:").grid(
        ...
        ttk.Button(input_frame, text="参照", command=self.select_input_dir).grid(
            row=0, column=1
        )

        # 出力ディレクトリ選択
        ttk.Label(main_frame, text="出力ディレクトリ:").grid(
        ...
        self.output_entry = ttk.Entry(
            output_frame, textvariable=self.output_var, width=60
        )
        self.output_entry.grid(row=0, column=0, sticky=(tk.W, tk.E), padx=(0, 5))

        ttk.Button(output_frame, text="参照", command=self.select_output_dir).grid(
            row=0, column=1
        )
        ...
        # 検出手法選択
        ttk.Label(settings_frame, text="検出手法:").grid(
            row=0, column=0, sticky=tk.W, pady=5
        )
        ...
        # 検出手法の説明
        method_info = ttk.Label(
            settings_frame,
            text="auto: 利用可能な全手法を精度順に試行（推奨）",
            font=("", 9),
            foreground="gray",
        )
        ...
        # モザイク設定
        ttk.Label(settings_frame, text="モザイクの粗さ:").grid(
        ...
        self.process_button = ttk.Button(
            button_frame, text="処理開始", command=self.start_processing
        )
        ...
        ttk.Button(
            button_frame, text="システム情報", command=self.show_detailed_system_info
        ).grid(row=0, column=2, padx=5)

        ttk.Button(button_frame, text="Proxy情報", command=self.show_proxy_info).grid(
            row=0, column=3, padx=5
        )

        ttk.Button(button_frame, text="終了", command=self.root.quit).grid(
    ...
    def show_detailed_system_info(self):
        ...
        text_widget.insert(tk.END, "\n=== 削除された手法 ===\n")
        text_widget.insert(
            tk.END, "✗ MediaPipe Face Detection (精度不十分により削除)\n"
        )
    ...
    def show_proxy_info(self):
        ...
        try:
            ...
            if proxy_manager.is_proxy_enabled():
                ...
                text_widget.insert(
                    tk.END, f"HTTP Proxy: {proxy_info.get('http_proxy', 'なし')}\n"
                )
                text_widget.insert(
                    tk.END, f"HTTPS Proxy: {proxy_info.get('https_proxy', 'なし')}\n"
                )
                text_widget.insert(
                    tk.END, f"No Proxy: {proxy_info.get('no_proxy', 'なし')}\n"
                )
                ...
                text_widget.insert(
                    tk.END, "環境変数でProxy設定が検出されませんでした。\n"
                )
                text_widget.insert(
                    tk.END,
                    "企業環境等でProxyが必要な場合は、以下の環境変数を設定してください:\n\n",
                )
                text_widget.insert(tk.END, "HTTP_PROXY=http://proxy.company.com:8080\n")
                text_widget.insert(
                    tk.END, "HTTPS_PROXY=http://proxy.company.com:8080\n"
                )
    ...
    def validate_inputs(self):
        ...
        if not os.path.exists(input_dir):
            messagebox.showerror(
                "エラー", f"入力ディレクトリが存在しません:\n{input_dir}"
            )
            return False

        if not os.path.isdir(input_dir):
            messagebox.showerror(
                "エラー", f"入力パスがディレクトリではありません:\n{input_dir}"
            )
            return False
        ...
        try:
            ...
            for i, image_path in enumerate(image_files):
                ...
                try:
                    ...
                    # 画像処理
                    if self.processor.process_single_image(
                        image_path, output_path, ratio
                    ):
                        success_count += 1
...
def main():
    ...
    if sys.platform == "darwin":
        try:
            # macOSでのフォーカス問題を回避
            os.system(
                "osascript -e 'tell application \"System Events\" to set frontmost of every process whose unix id is {} to true'".format(
                    os.getpid()
                )
            )
        except:
